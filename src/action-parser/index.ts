...
function smartResizeForV15(height: number, width: number): [number, number] {
  const maxSide = Math.max(height, width);
  const minSide = Math.min(height, width);
  
  if (maxSide <= MAX_PIXELS_V1_5) {
    return [IMAGE_FACTOR, IMAGE_FACTOR];
  }
  
  const ratio = MAX_PIXELS_V1_5 / maxSide;
  const newMinSide = Math.round(minSide * ratio);
  
  if (newMinSide < MIN_PIXELS) {
    const adjustedRatio = MIN_PIXELS / minSide;
    const newMaxSide = Math.round(maxSide * adjustedRatio);
    
    if (height > width) {
      return [IMAGE_FACTOR * (newMaxSide / height), IMAGE_FACTOR * (MIN_PIXELS / width)];
    } else {
      return [IMAGE_FACTOR * (MIN_PIXELS / height), IMAGE_FACTOR * (newMaxSide / width)];
    }
  }
  
  if (height > width) {
    return [IMAGE_FACTOR * ratio, IMAGE_FACTOR * ratio];
  } else {
    return [IMAGE_FACTOR * ratio, IMAGE_FACTOR * ratio];
  }
}
