...
import { Operator, ScreenshotOutput, ExecuteParams, ExecuteOutput } from '../sdk/types';
import { StatusEnum } from '../shared/types';
import { parseBoxToScreenCoords } from '../shared/utils';
...
export class AdbOperator extends Operator {
  ...
  public async screenshot(): Promise<ScreenshotOutput> {
    ...
    try {
      ...
      console.error('[AdbOperator] Screenshot error:', error);
      throw error;
    }
  }
  ...
  async execute(params: ExecuteParams): Promise<ExecuteOutput> {
    ...
    try {
      switch (action_type) {
        ...
        default:
          console.warn(`[AdbOperator] Unsupported action: ${action_type}`);
          break;
      }
    } catch (error) {
      console.error('[AdbOperator] Error:', error);
      throw error;
    }
